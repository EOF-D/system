...
/**
 * Dashboard page displaying courses and invitations.
 */
function DashboardPage() {
  // Auth context.
  const { user, isLoggedIn } = useAuth();

  // Courses and invitations.
  const [courses, setCourses] = useState<ProfessorCourse[] | StudentCourse[]>(
    [],
  );
  ...
  const [selectedCourse, setSelectedCourse] = useState<ProfessorCourse | null>(
    null,
  );
  ...
  const fetchCourses = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        setCourses(response.data as ProfessorCourse[] | StudentCourse[]);
      } else {
  ...
  const handleOpenInviteModal = (course: ProfessorCourse) => {
  ...
  const isProfessorCourse = (course: any): course is ProfessorCourse => {
    return "enrollment_count" in course;
  };

  const isStudentCourse = (course: any): course is StudentCourse => {
    ...
    return (
      <div className="mb-8">
        ...
        {isInvitationsLoading ? (
          ...
          <div className="bg-default-100 rounded-lg p-6 text-center mb-8">
  ...
  const renderCourseCard = (course: ProfessorCourse | StudentCourse) => {
    return (
      <Card
        key={course.id}
            ...
            <Dropdown>
              ...
              <DropdownMenu aria-label="Course actions">
                <DropdownItem
                  ...
                  onPress={() =>
                    handleOpenInviteModal(course as ProfessorCourse)
                  }
                >
                  Invite Students
                </DropdownItem>
